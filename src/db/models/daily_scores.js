...
async getScore(guildId, discordId, season) {
    try {
      if (season !== 0) {
        ...
        return await this.db.get(
          `SELECT UserScore.discord_id,
                  UserScore.streak,
                  UserScore.best_streak,
                  SUM(SeasonScores.score)    AS score,
                  SUM(SeasonScores.attempts) AS attempts
           FROM UserScore
                  LEFT JOIN SeasonScores
                            ON UserScore.discord_id = SeasonScores.discord_id
                              AND UserScore.guild_id = SeasonScores.guild_id
           WHERE UserScore.guild_id = @guildId
             AND UserScore.discord_id = @discordId
           GROUP BY UserScore.discord_id, UserScore.guild_id
           LIMIT 1;`,
          { guildId, discordId },
        );
      }
